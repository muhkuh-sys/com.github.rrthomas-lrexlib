...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.1-lrexlib-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.1-lrexlib-*.xml'))
if len(astrMatch) == 0:
    raise Exception('No match found for "lua5.1-lrexlib-*.xml".')
elif len(astrMatch) > 1:
    raise Exception('More than one match found for "lua5.1-lrexlib-*.xml". Clean your build folder.')
...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.4-lrexlib-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.4-lrexlib-*.xml'))
if len(astrMatch) == 0:
    raise Exception('No match found for "lua5.4-lrexlib-*.xml".')
elif len(astrMatch) > 1:
    raise Exception('More than one match found for "lua5.4-lrexlib-*.xml". Clean your build folder.')
